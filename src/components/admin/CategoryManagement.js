...
const CategoryManagement = ({ 
  ...
  const handleSaveEdit = () => {
    if (editingCategory.name && editingCategory.code) {
      ...
      setJewelryCategories(prev => 
        prev.map(cat => 
          cat.id === editingCategoryId 
            ? { ...editingCategory, id: editingCategoryId }
            : cat
        )
      );
  ...
  const handleDeleteCategory = (id) => {
    ...
    // Check if category has jewelry items
    const fullCategoryName = category.parentId 
      ? `${jewelryCategories.find(p => p.id === category.parentId)?.name} - ${category.name}`
      : category.name;
    
    const jewelryCount = jewelryPieces.filter(j => 
      (j.category === category.name || j.category === fullCategoryName) && 
      j.status !== 'Archived'
    ).length;
    
    if (jewelryCount > 0) {
      alert(`Cannot delete "${category.name}" because it has ${jewelryCount} jewelry items. Please reassign or remove those items first.`);
      return;
    }
  ...
  const getJewelryCount = (category) => {
    const fullCategoryName = category.parentId 
      ? `${jewelryCategories.find(p => p.id === category.parentId)?.name} - ${category.name}`
      : category.name;
    
    return jewelryPieces.filter(j => 
      (j.category === category.name || j.category === fullCategoryName) && 
      j.status !== 'Archived'
    ).length;
  };
  ...
  const renderCategoryRow = (category, level = 0) => {
    ...
    const jewelryCount = getJewelryCount(category);
        ...
        <tr className={`hover:bg-gray-50 ${level > 0 ? 'bg-blue-50' : ''}`}>
          ...
          <td className="px-6 py-4">
            <span className={`px-2 py-1 rounded text-xs font-medium ${
              isParent ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
            }`}>
              {jewelryCount} items
            </span>
          </td>
    ...
    <div className="p-6">
        ...
        <div className="mb-6 bg-blue-50 p-6 rounded-lg border">
              ...
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setNewCategory(prev => ({...prev, parentId: null, type: 'parent'}));
                  }}
                  className="mr-2"
                />
                <Gem className="mr-1" size={16} />
                <span>Material Type (Parent)</span>
              </label>
              <label className="flex items-center">
                <input
                  type="radio"
                  ...
                  onChange={(e) => {
                    setCategoryType(e.target.value);
                    setNewCategory(prev => ({...prev, type: 'child'}));
                  }}
                  className="mr-2"
                />
                <Award className="mr-1" size={16} />
                <span>Jewelry Type (Child)</span>
              </label>
            </div>
          </div>
          ...
          <div className="flex space-x-2 mt-4">
            <button
              ...
              disabled={!newCategory.name || !newCategory.code || (categoryType === 'child' && !newCategory.parentId)}
              className={`px-4 py-2 rounded font-medium flex items-center space-x-1 ${
                newCategory.name && newCategory.code && (categoryType === 'parent' || newCategory.parentId)
                  ? 'bg-green-500 text-white hover:bg-green-600'
                  : 'bg-gray-300 text-gray-500 cursor-not-allowed'
              }`}
            >
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-purple-50 p-4 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-purple-600 text-sm font-medium">Active Jewelry</p>
      ...
      <div className="mt-6 bg-yellow-50 p-4 rounded-lg border border-yellow-200">
        ...
        <div className="text-sm text-yellow-700 space-y-1">
          <p>• <strong>Material Types (Parent):</strong> Diamond, Gold, Kundan, etc. - These are your main material categories</p>
          <p>• <strong>Jewelry Types (Child):</strong> Necklace, Ring, Earring, etc. - These are subcategories under each material</p>
          <p>• <strong>Expand/Collapse:</strong> Click the arrow to expand parent categories and see their jewelry types</p>
          <p>• <strong>Code Format:</strong> Use clear codes like DIA-N (Diamond Necklace), GOLD-R (Gold Ring)</p>
          <p>• <strong>Charges:</strong> Set material-specific wastage and making charges for accurate pricing</p>
          <p>• <strong>Delete Protection:</strong> Cannot delete categories with existing jewelry or subcategories</p>
        </div>
      </div>
    </div>
  );
