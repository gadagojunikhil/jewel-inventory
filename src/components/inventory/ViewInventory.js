...
import { 
  ...
  FileText, Plus, Copy, Printer
} from 'lucide-react';

import api from '../../services/api';

// Uses these API calls:
...
const ViewInventory = () => {
  ...
  const filteredAndSortedItems = useMemo(() => {
    ...
    // Price range filter
    if (priceRange.min) {
      filtered = filtered.filter(item => item.salePrice >= parseFloat(priceRange.min));
    }
    if (priceRange.max) {
      filtered = filtered.filter(item => item.salePrice <= parseFloat(priceRange.max));
    }

    // Date range filter
    if (dateRange.start) {
      filtered = filtered.filter(item => 
        new Date(item.createdDate) >= new Date(dateRange.start)
      );
    }
    if (dateRange.end) {
      filtered = filtered.filter(item => 
        new Date(item.createdDate) <= new Date(dateRange.end)
      );
    }
  ...
  const bulkDelete = async () => {
    try {
      ...
      await Promise.all(selectedItems.map(id => api.deleteJewelry(id)));
      // Reload jewelry pieces from backend
      const updatedJewelry = await api.getJewelry();
      setJewelryPieces(updatedJewelry);
      setSelectedItems([]);
